...
class SectionCut(object):
    ...
    def calculate_cuts(self, nums: list[int], target: list[int]) -> int:
        # begin
        a, b = sorted(_ for _ in nums if _ % 2), sorted(_ for _ in nums if _ % 2 == 0)
        c, d = sorted(_ for _ in target if _ % 2), sorted(_ for _ in target if _ % 2 == 0)
        return sum((i - j) // 2 for i, j in zip(a, c) if i > j) + sum((i - j) // 2 for i, j in zip(b, d) if i > j)
        # end

f = SectionCut().calculate_cuts
# input = read_input()
# print(f(*input))  # [1,1,2,2,2,2,2,2,2,1]
print(f(nums=[8,12,6], target=[2,14,10]))  # 2
print(f(nums=[1,2,5], target=[4,1,3]))  # 1
print(f(nums=[1,1,1,1,1], target=[1,1,1,1,1]))  # 0
print(f(nums=[2, 100], target=[4, 98]))