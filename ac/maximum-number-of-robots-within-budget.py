...
class SectionCut(object):
    ...
    def calculate_cuts(self, chargeTimes: list[int], runningCosts: list[int], budget: int) -> int:
        # begin
        """
        ...
        n = len(chargeTimes)
        charges = SortedList()
        charge_cost, running_cost = 0, 0

        for i in range(n):
            charges.add(chargeTimes[i])
            charge_cost = charges[-1]
            running_cost += runningCosts[i]
            while charges and charge_cost + len(charges) * running_cost > budget:
                j = i - len(charges) + 1
                charges.remove(chargeTimes[j])
                charge_cost = charges[-1] if charges else 0
                running_cost -= runningCosts[j]
            res = max(res, len(charges))
        return res
        # end
...
print(f(chargeTimes=[3,6,1,3,4], runningCosts=[2,1,3,4,5], budget=25))  # 3
# print(f(chargeTimes=[11,12,19], runningCosts=[10,8,7], budget=19))  # 0